...
/**
 * External calendar sync via ICS/iCal URLs. Fetches each enabled source
 * sequentially via curl, parses ICS to JSON via the bundled
 * calendar_ics.js, merges events into a single timeline, and persists
 * a JSON cache between sessions.
 *
 * Public API:
 *   events                 — merged event list (sorted by startDate by callers)
 *   sourceStatuses         — per-source { error, lastFetch, eventCount }
 *   ready, fetching        — state booleans
 *   getEventsForDate(d)    — events on a specific calendar day
 *   getUpcomingEvents(d)   — events in the next N days
 *   getSourceColorsForDate(d) — distinct source colors for that day
 *   addSource / removeSource / updateSource / toggleSource
 *   refreshSource / forceRefreshAll
 *
 * Signals: eventsUpdated, fetchStarted, fetchFinished(success),
 *          sourceError(sourceId, error)
 *
 * Service is dormant when Config.options.calendar.externalSync.enable is
