...
MouseArea {
    ...
    function resetDrag() { _dragDiffX = 0; _dragDiffY = 0 }

    onPressed: (mouse) => {
        if (!root.interactive) { if (mouse.button === Qt.LeftButton) mouse.accepted = false; return }
        if (mouse.button === Qt.LeftButton) { startX = mouse.x; startY = mouse.y }
    }
    onReleased: (mouse) => {
        if (!root.interactive) return
        dragging = false
        root.dragReleased(_dragDiffX, _dragDiffY)
        if (root.automaticallyReset) root.resetDrag()
    }
    ...
    onCanceled: (mouse) => { if (!root.interactive) return; released(mouse) }
}
